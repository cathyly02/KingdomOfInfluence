...
@onready var npc_script = load("root/Scenes/npc.gd")

var wander_direction = Vector3.ZERO
var wander_timer = 0.0
var interacting = false  # New flag to track interaction
...
func _physics_process(delta):
	...
	# Check if interaction started
	if distance_to_player < detect_radius and Input.is_action_pressed("interact"):
		interacting = true
	...
	if interacting:
		...
		var face_direction = -to_player.normalized()
		velocity.x = 0.0
		velocity.z = 0.0
		rotate_towards(face_direction, delta)
	else:
		# Wander randomly
		wander_timer -= delta
		if wander_timer <= 0:
